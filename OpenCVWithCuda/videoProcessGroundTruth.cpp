...
实验的ground truth ， 手动标记每一帧图片

代码 参考videoProcessOffline.cpp
*/
    ...
    Mat prevgray, gray, flow, cflow, frame,binaryPic,binaryPic2;
	 ...
	 float TakeTime;
        ...
        while (cap.get(CV_CAP_PROP_POS_FRAMES)< cap.get(CV_CAP_PROP_FRAME_COUNT)) {
			...
			cout<<"Current frame Num :"<<frameNum ; 
            ...
            putText(frame, "hi:"+intToString(frameNum), Point(50,100), FONT_HERSHEY_SIMPLEX, 1, Scalar(0,200,200), 4);
            imshow("FLOW", frame);
